...
const DashboardWithDatabase = () => {
  ...
  const [activityNextCursor, setActivityNextCursor] = useState<number | null>(null);
  ...
  const loadActivityLog = async (cursor?: number) => {
                      ...
                      <ListItemText
                        primary={
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                            ...
                            {(() => {
                              const cName = entry.companyName || reportsData?.companies?.find((c: any) => String(c.id) === String(entry.companyId))?.name;
